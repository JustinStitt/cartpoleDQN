...
class DeepQNetwork(nn.Module):
    def __init__(self, lr, input_dims, fc1_dims,fc2_dims,n_actions):
        ...
        self.device = T.device('cuda:0' if T.cuda.is_available() else 'cpu')#uses GPu if available, else cpu
...
class Agent(object):
    ...
    def learn(self):
        if self.mem_cntr > self.batch_size:#only learn when you have enough memories to fill batch_size
            ...
            loss.backward()#loss.backward() computes dloss/dx for every parameter x which has requires_grad=True. These are accumulated into x.grad for every parameter x. In pseudo-code: x.grad += dloss/dx
            self.Q_eval.optimizer.step()#this is what changes our parameters
